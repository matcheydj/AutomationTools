...
open my $f,">>/etc/profile";

print $f <<'end';
dev=$(df |awk '$NF~/^\/$/{print $1}')
vol=$(diskutil info / |awk -F: '/Volume Name/{gsub(" *","",$2);print $2}')
dname=$(sw_vers|awk -F: 'NR==2{gsub("[[:space:]]*","",$2);print $2}')
if [ "$vol" != "$dname" ]; then
...
unless($opt_i){
    print "-i option is not specified. \n";
    exit 12;
}

my $reply;
if (-e "/Library/Application\ Support/LANDesk"){
   print "LANDesk Agent is installed, delete it again?";
   $reply=<STDIN>;
}
if($reply =~ /yes|y|Y|Yes/){
open my $a,">a.$$";
print $a <<'end';
#!/bin/bash
#####################################################
...
my $url="http://${opt_i}/ldlogon/mac/LDMSClient.mpkg.zip";
$ua->get($url,":content_file"=>"LDMSClient.mpkg.zip");
system("unzip","LDMSClient.mpkg.zip");
system("sudo","installer","-pkg","LDMSClient.mpkg","-target","/","-verbose");
print "ok, no error";
