...
local,headers=urllib.request.urlretrieve("https://support.office.com/en-us/article/Release-notes-for-Office-2016-for-Mac-ed2da564-6d53-4542-9954-7e3209681a41")

soup=BeautifulSoup(open(local,'rt',encoding="utf-8"),"html.parser")     

table=soup.find_all("table","banded")[0]

for i in re.finditer("(http://[0-9a-zA-Z/\\\_\\\.\\\-].*.pkg)",str(table)):
    name=i.group(0).split("/")[-1]
    length=urllib.request.urlopen(i.group(0)).info()["content-length"]
#    urllib.retrieve(i.group(0))
